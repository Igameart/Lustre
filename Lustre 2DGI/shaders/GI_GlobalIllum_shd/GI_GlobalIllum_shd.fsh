...
float map(vec2 uv, bool am_in, out float hit_data, out bool inside, out float inner_emis)
{
	...
	if (inside == true){
			inside = bool( texture2D(u_flood_data, uv).b );
	}
...
void main() 
{
	...
	// to normalise it.
	//emis *= (1.0 / float(Iu_rays_per_pixel));
	...
	if (inner_glow > 0.0){
		...
		col += (inner_glow * u_emission_multi) * (texture2D(u_scene_colour_data,UV).rgb) * att;
	}
	
	gl_FragColor = vec4(col, 1.0) * texture2D(u_scene_colour_data,UV);
